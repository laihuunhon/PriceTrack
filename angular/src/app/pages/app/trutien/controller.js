export default class TruTienController {
  constructor($scope, $timeout, SocketService) {
    ...
    Object.assign(this, { $scope, $timeout, SocketService })
    ...
    this.bookmarks = []
    this.loadBookmarkFromLocalStorage()
    ...
    const columnDefs = [{
      ...
      cellRenderer: (params) => {
        const filter = this.bookmarks.filter(item => item._id === params.value)
        if (filter.length > 0) {
          return `<i class="fa fa-star fa-2x" style="cursor: pointer" ng-click="ctrl.removeBookmark(data)"></i>`
        } else {
          return `<i class="fa fa-star-o fa-2x" style="cursor: pointer" ng-click="ctrl.saveBookmark(data)"></i>`
        }
      }
    }, {
      headerName: "Trận",
    ...
    this.bookmarkGridOptions = {
      columnDefs,
      angularCompileRows: true,
      rowHeight: 75,
      enableColResize: true,
      rowData: this.bookmarks,
      overlayLoadingTemplate: '<span class="ag-overlay-loading-center">Loading</span>',
    }
    this.sortList = [
      '',
    ...
    SocketService.on('SERVER_RESPONSE', (response) => {
      $scope.$apply(() => {
        console.log(response.result)
        this.gridOptions.api.setRowData(response.result)
        this.gridOptions.api.hideOverlay()
        // this.fixedCalculateCombineProperty = response.result;
      });
    });
    this.SocketService.emit('GET_DATA', {
      sortBy: {
        relationCount: -1
      }
    })
  ...
  loadBookmarkFromLocalStorage() {
    if (localStorage.tranlinhBookmarks) {
      this.bookmarks = JSON.parse(localStorage.tranlinhBookmarks)
    }
  }
  saveBookmark(data) {
    this.bookmarks.push(data)
    this.bookmarkGridOptions.api.setRowData(this.bookmarks)
    this.gridOptions.api.refreshView()
    localStorage.tranlinhBookmarks = JSON.stringify(this.bookmarks)
  }
  removeBookmark(data) {
    this.bookmarks = this.bookmarks.filter(item => item._id !== data._id)
    this.bookmarkGridOptions.api.setRowData(this.bookmarks)
    this.gridOptions.api.refreshView()
    localStorage.tranlinhBookmarks = JSON.stringify(this.bookmarks)
  }
  ...
  multipleSort() {
    ...
    const sortBy = {}
    if (this.sortBy1) {
      sortBy[this.sortBy1] = -1
    }
    if (this.sortBy2) {
      sortBy[this.sortBy2] = -1
    }
    if (this.sortBy3) {
      sortBy[this.sortBy3] = -1
    }
    if (this.sortBy4) {
      sortBy[this.sortBy4] = -1
    }
    this.SocketService.emit('GET_DATA', {
      sortBy,
    })
